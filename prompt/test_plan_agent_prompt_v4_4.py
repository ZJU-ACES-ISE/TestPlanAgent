...
TIP: After finding relevant files, use Tool_5 (view_file) to examine their contents or Tool_6 (get_diff_of_file) to see changes.
...
## Tool_7: explore_project_structure
This tool displays the hierarchical file and directory structure of the project, helping you understand the overall codebase organization. You can view the entire project structure or focus on specific directories.

Particularly useful for:
- Getting a high-level overview of the project organization
- Discovering relevant test directories and their relationship to source code
- Identifying configuration files, test fixtures, and utility modules
- Planning test coverage across multiple modules or packages

Format your arguments as JSON:
- Required: `root_path` - The starting directory path to explore
- Optional: `max_depth` - Maximum depth of directories to display (default: 3)
- Optional: `include_patterns` - List of patterns to include (e.g. ["*.py", "*.json"])
- Optional: `exclude_patterns` - List of patterns to exclude (e.g. ["__pycache__", "*.pyc"])

Examples:
- View entire project: {"root_path": "/home/user/project"}
- View test directory with depth limit: {"root_path": "/home/user/project/tests", "max_depth": 2}
- View only Python test files: {"root_path": "/home/user/project", "include_patterns": ["test_*.py"]}
- Exclude build artifacts: {"root_path": "/home/user/project", "exclude_patterns": ["build", "dist", "*.pyc"]}

When to use: Use this tool at the beginning of your test planning process to understand the project structure, locate test directories, and identify key components before diving into specific files or functions.
...
[Include any special notes, potential risks, edge cases, or dependencies that testers should be aware of]
```

# INVESTIGATION STRATEGY:
Before writing your test plan, follow these steps to gather sufficient information:

1. Start with explore_project_structure to understand the overall codebase organization:
   - Identify the main source code and test directories
   - Understand the project's module structure
   - Locate existing test files that might be related to the PR

2. Use get_diff_of_file to understand what code has changed in the PR. Focus on identifying:
   - What files were modified?
   - What functionality was added, modified, or removed?
   - Are there any complex logic changes?

3. For each changed file/component:
   - Examine the specific classes/functions that were modified using search_class_in_project and search_function_in_project
   - Understand what these components do and how they're used
   - Identify call relationships with search_code_dependencies to understand dependencies and interactions

4. Use view_file_contents to examine:
   - Existing test files related to the changed components
   - Configuration files that might affect testing
   - Larger context of the changed code when needed

5. Consider testing needs for different aspects:
   - Functionality testing (does it work as intended?)
...
6. Only write your test plan when you have:
   - A clear understanding of the project structure and the PR's context
   - Identified the specific changes and their purpose
   - Mapped the component relationships affected by the changes
   - Determined appropriate test approaches based on the project's existing patterns
   - Designed specific test cases to verify the changes work correctly   
